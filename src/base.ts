...
interface OneAuthWallet {
    postMessage(...args: unknown[]): unknown;
}

interface OneAuthWindow extends Window {
    oneauth?: OneAuthWallet;
}

declare const window: OneAuthWindow;

export interface OneAuthWalletAdapterConfig {
    provider?: string | OneAuthWallet;
    network?: WalletAdapterNetwork;
    timeout?: number;
}

export abstract class BaseOneAuthWalletAdapter extends BaseMessageSignerWalletAdapter {
    protected _provider: string | OneAuthWallet | undefined;
    ...
    constructor({ provider, network = WalletAdapterNetwork.Mainnet, timeout = 10000 }: OneAuthWalletAdapterConfig = {}) {
        super();
        ...
        if (this._readyState !== WalletReadyState.Unsupported) {
            if (typeof this._provider === 'string') {
                ...
                scopePollingDetectionStrategy(() => {
                    if (typeof window.oneauth?.postMessage === 'function') {
    ...
    async connect(): Promise<void> {
        ...
        try {
            ...
            const provider = this._provider || window!.oneauth!;
